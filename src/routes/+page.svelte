<svelte:head>
  ...
  <meta property="og:description"
        content="Découvrez les pages de {pages.slice(0, 7).map(el => el.author).join(', ')}...">
...
<div class="h-screen w-screen flex items-center flex-col justify-center content-center bg-cody">
  ...
  <main class="bg-gray-800 text-white rounded-md shadow-2xl overflow-x-hidden overflow-y-auto">
    <input maxlength="25" bind:value={name} placeholder="👤 Your name" class="bg-gray-600 p-6 w-full text-center"
           type="text">
    <ul>
      <li>
      ...
      {#each pages as page}
        <li>
          <a sveltekit:prefetch class="p-6 hover:bg-gray-700 cursor-pointer flex flex-row"
             href={`/${page.author}/${name}`}>
...
let name = `pseudoCool${ Math.floor(Math.random() * 1000 + 1000) }`;
let pages = getPages();
</script>
